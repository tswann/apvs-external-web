  ...
  if (req.file) {
    clam.scan(req.file.path).then((infected) => {
      try {
        ...
        ClaimDocumentInsert(ids.reference, ids.eligibilityId, claimId, req.fileUpload).then(function (claimDocumentId) {
          if (req.query.document === 'RECEIPT') {
            InsertTask(ids.reference, ids.eligibilityId, claimId, taskEnum.CHECK_DOCUMENT, claimDocumentId).then(function () {
